...
export class AddCourseComponent implements OnInit {
  ...
  saveCourse() {
    this.http.post("/api/courses/" + this.user.id, {'name' : this.course.courseName, 'isValidated' : true}).subscribe(courseId => {
  ...
  saveQuestions(courseId) : Observable<any> {
    ...
    for (let i = 0; i < this.course.questions.length; i++) {
      x = this.http.post("/api/courses/"+ courseId +"/questions", {"question" : this.course.questions[i].question, "questionType" : this.course.questions[i].questionType });
  ...
  saveAnswers(i, courseId, questionId) {
    for (let j = 0; j < this.course.questions[i].questionAnswers.length; j++) {
      this.http.post("/api/courses/"+ courseId + "/questions/" + questionId + "/answers", {"answer" : this.course.questions[i].questionAnswers[j].answer, "isRight" : this.course.questions[i].questionAnswers[j].isRight}).subscribe(answerId => {});
    }
  }
  ...
  saveTopics(courseId) : Observable<any> {
    let x;
    for (let i = 0; i < this.course.topics.length; i++) {
      x = this.http.post("/api/courses/" + courseId + "/topics", {"name" : this.course.topics[i]});
      x.subscribe(topicId => {});
    }
    return x;
  }
  ...
  saveVideos(courseId) : Observable<any> {
    ...
    for (let i = 0; i < this.course.videos.length; i++) {
      ...
      x = this.http.post("/api/courses/" + courseId + "/videos", {"name" : this.course.videos[i].name, "video" : this.course.videos[i].url, "description": this.course.videos[i].description});
      x.subscribe(videoId => {});
    }
    return x;
  }
